"""
BRCA2 Variant Pathogenicity Predictor v2.1
==========================================
Auto-fetches variant data from gnomAD and ClinVar APIs
Fixed gnomAD API query for current schema
...
def fetch_gnomad_data(variant_id):
    ...
    try:
        ...
        # gnomAD GraphQL query - FIXED: removed 'af' from populations
        query = """
...
def parse_gnomad_data(variant_data):
    ...
    # Calculate frequencies from ac/an
    if total_an > 0:
    ...
    if consequences:
        for tc in consequences:
            if tc.get('gene_symbol') == 'BRCA2':
                ...
                if consequence in severe_consequences or tc.get('lof') == 'HC':
                    features['consequence_severity'] = 2
                elif consequence in moderate_consequences:
                    features['consequence_severity'] = 1
                else:
                    features['consequence_severity'] = 0
                
                # Try to get protein position from hgvsp
                hgvsp = tc.get('hgvsp', '')
                if hgvsp:
...
def parse_clinvar_data(clinvar_data):
    ...
    # Fallback to clinical_significance field
    clin_sig = clinvar_data.get('clinical_significance', {})
    if isinstance(clin_sig, dict) and not germline:
...
with st.sidebar:
    ...
    st.markdown("""
    **Version 2.1** - Auto-fetch enabled
    
    Enter a gnomAD ID or ClinVar ID to automatically retrieve all variant data.
        ...
        if not gnomad_id and not clinvar_id:
            ...
            if fetch_results:
                ...
                with f2:
                    ...
                    st.write(f"• Review: `{all_features['ReviewStatus_numeric']} stars`")
...
with tab2:
    ...
    with col2:
        review_m = st.selectbox("Review Status", [0, 1, 2, 3, 4], index=1)
        subs_m = st.slider("Submitters", 1, 20, 1)
...
# Footer
st.markdown("---")
st.markdown("<p style='text-align: center; color: #666;'>BRCA2 Database Bias Project v2.1</p>", unsafe_allow_html=True)
